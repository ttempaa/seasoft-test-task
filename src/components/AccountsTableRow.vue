...
function areInputsValid(): boolean {
  return localAccount.login.length > 0 && (
...
function prepareValues() {
  localAccount.marks = stringMarks.value.split(marksSeparator).filter(Boolean).map((mark) => ({ text: mark.trim() }))
...
<template>
  <tr>
    <td>
      <n-input v-model:value="stringMarks" @blur="save()" type="text" placeholder="Значение" maxlength="50" />
    </td>
    <td>
      <n-select v-model:value="localAccount.type" @update:value="save()" :options="accountTypeSelectOptions" />
    </td>
    <td>
      <n-input v-model:value="localAccount.login" @blur="save()" type="text" placeholder="Значение" maxlength="100"
        :status="localAccount.login.length > 0 ? 'success' : 'error'" />
    </td>
    <td>
      <n-input v-if="localAccount.type === AccountType.LOCAL" v-model:value="localAccount.password" @blur="save()"
        type="password" placeholder="Значение" maxlength="100"
        :status="localAccount.password !== null && localAccount.password.length > 0 ? 'success' : 'error'" />
    </td>
    <td>
